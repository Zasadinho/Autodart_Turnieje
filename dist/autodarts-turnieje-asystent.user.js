...
(function () {
    ...
    return Object.freeze({
      [X01_PRESET_PDC_EUROPEAN_TOUR_OFFICIAL]: Object.freeze({
        ...
        notes: Object.freeze([
          ...
          "Max Rund 50 remains a technical AutoDarts limit and is not part of the PDC rule claim.",
        ]),
        apply: Object.freeze({
          mode: "ko",
          bestOfLegs: 11,
  ...
  function normalizeTournament(rawTournament, fallbackKoDrawLocked = true) {
    ...
    return {
      ...
      name: normalizeText(rawTournament.name || "Lokalny Turniej"),
      mode,
      ko: mode === "ko"
...
// App layer: runtime orchestration, persistence scheduling and user feedback.
  ...
  function runSelfTests() {
    ...
    try {
      ...
      record(
        "KO Open Draw: przydzielonymi wolnymi losami",
        deterministic && byeCount > 0,
        `matches=${openDrawMatches.length}, byes=${byeCount}, deterministic=${deterministic}`,
      );
    } catch (error) {
      record("KO Open Draw: przydzielonymi wolnymi losami", false, String(error?.message || error));
    }
    ...
    try {
      ...
      record(
        "KO 6: pełne drzewo dla 8 zawodników z 2 wolnymi losami",
      ...
      record("KO 6: pełne drzewo dla 8 zawodników z 2 wolnymi losami", false, String(error?.message || error));
    }
    ...
    try {
      ...
      const byeCount = matches.filter((match) => isByeMatchResult(match)).length;
      record(
        "KO 8: od startu istnieje 7 meczów w drzewie",
        matches.length === 7 && byeCount === 0,
        `matches=${matches.length}, byes=${byeCount}`,
      );
    } catch (error) {
      record("KO 8: od startu istnieje 7 meczów w drzewie", false, String(error?.message || error));
    }
    ...
    try {
      ...
      record(
        "Schematy: European Tour + Basic w pełni zwalidowany",
      ...
      record("Schematy: European Tour + Basic w pełni zwalidowany", false, String(error?.message || error));
    }
    ...
    try {
      ...
      record(
        "„Ustawienie Presetu: European Tour Official wymaga KO + Best of 11 + 501/SI/DO",
        compliant && !wrongBestOf,
        `official=${compliant}, wrongBestOf=${wrongBestOf}`,
      );
    } catch (error) {
      record("„Ustawienie Presetu: European Tour Official wymaga KO + Best of 11 + 501/SI/DO", false, String(error?.message || error));
    }

    {
      ...
      try {
        ...
        if (!(createForm instanceof HTMLFormElement) || !(presetSelect instanceof HTMLSelectElement)) {
          throw new Error("Nie wybrano formularza ani presetu.");
        }
        ...
        record(
          "Preset UI: wybranie i zastosowanie presetu uzupełnia wszystkie pola formularza zgodnie z konfiguracją",
        ...
        record("Preset UI: wybranie i zastosowanie presetu uzupełnia wszystkie pola formularza zgodnie z konfiguracją", false, String(error?.message || error));
      } finally {
    ...
    try {
      ...
      record(
        "Tworzenie turnieju -> payload startu meczu przejmuje X01 + Best of w sposób spójny",
      ...
      record("Tworzenie turnieju -> payload startu meczu przejmuje X01 + Best of w sposób spójny", false, String(error?.message || error));
    }
...
try {
  ...
  let lobby = null;
  try {
    logDebug("api", "Tworzenie lobby z payloadem turniejowym.", {
      matchId: match.id,
    ...
    // Fallback dla wariantów walidacji backendu, które odrzucają wybrane wartości bullMode.
    ...
    logWarn("api", "Ponawianie tworzenia lobby z bullMode 25/50.", {
      matchId: match.id,
      legs: lobbyPayload.legs,
      settings: lobbyPayload.settings,
    });
...
async function syncPendingApiMatches() {
  ...
  if (!token) {
    ...
    if (shouldShowAuthNotice()) {
      setNotice("error", "Auto-Sync wstrzymany: nie znaleziono tokenu autoryzacji. Zaloguj się ponownie.");
    }
    return;
  }
  ...
  try {
    for (const match of syncTargets) {
      ...
      if (syncOutcome.authError) {
        state.apiAutomation.authBackoffUntil = Date.now() + API_AUTH_NOTICE_THROTTLE_MS;
        if (shouldShowAuthNotice()) {
          setNotice("error", "Auto-Sync wstrzymany: autoryzacja wygasła. Proszę zalogować się ponownie.");
        }
        logWarn("api", "Błąd autoryzacji podczas auto‑sync.");
        return;
      }
      ...
      if (!syncOutcome.ok && syncOutcome.message && !syncOutcome.recoverable) {
        logWarn("api", `Auto-sync nie powiódł się dla ${match.id}: ${syncOutcome.message}`);
      }
    }
  } finally {
    ...
    root.innerHTML = `
      <div style="margin:10px 0 14px 0;padding:14px;border-radius:12px;border:1px solid rgba(120,203,255,0.45);background:linear-gradient(180deg, rgba(54,70,145,0.92), rgba(34,80,136,0.9));color:#f4f7ff;box-shadow:0 10px 24px rgba(7,11,25,0.28);">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:8px;">
          <div style="font-size:15px;line-height:1.2;font-weight:800;letter-spacing:0.25px;">Lokalny Turniej</div>
    ...
    try {
      ...
      if (syncOutcome.reasonCode === "completed" || syncOutcome.completed) {
        ...
        if (alreadyStored) {
          ...
          setNotice("success", syncOutcome.message || "Ergebnis wurde in Lokalny Turniej \u00fcbernommen.", 2600);
        }
...
// Presentation layer: UI rendering and interaction wiring.
  ...
  function renderTournamentDurationEstimate(estimate) {
    const helpLinks = renderInfoLinks([
      ...
      { href: README_SETTINGS_URL, kind: "tech", label: "Otwórz ustawienia profilu czasowego", title: "README: Ustawienia" },
    ]);
  ...
  function renderTournamentTab() {
    ...
    if (!tournament) {
      ...
      const presetOptions = [
        ...
        `<option value="${X01_PRESET_CUSTOM}" ${draft.x01Preset === X01_PRESET_CUSTOM ? "selected" : ""}>Indywidualny / Manuell</option>`,
      ].join("");
      ...
      return `
        <section class="ata-card tournamentCard">
          ${renderSectionHeading("Utwórz nowy turniej", createHeadingLinks)}
