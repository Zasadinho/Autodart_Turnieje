...
(function () {
  ...
  const TAB_META = Object.freeze([
    ...
    { id: "view", label: "Drabinka Turniejowa" },
...
// App layer: runtime orchestration, persistence scheduling and user feedback.
  ...
  function getAuthTokenFromCookie() {
    try {
      // 1. Token intercepted from the Autodarts app's fetch/XHR calls or postMessage refresh
      const intercepted = getInterceptedToken();
      if (intercepted) {
        return intercepted;
      }

      // 2. Authorization= cookie on play.autodarts.io (legacy fallback)
        ...
        ? `
          <div class="ata-match-editor">
            ...
            <div class="ata-editor-actions">
              <button type="button" class="ata-btn" data-action="save-match" data-match-id="${escapeHtml(match.id)}" title="${escapeHtml(saveHelpText)}">Zapisz</button>
